...
gx, gy = 0, 0
def init():
    global Ascore, Bscore, ACountDown, BCountDown, msg, graph, gx, gy
    Ascore, Bscore, ACountDown, BCountDown = 0, 0, 0, 0
    with open('88map') as f:
        mygraph = f.readlines()
    gy, gx = map(int, mygraph[0].split())
    print("y:{} x:{}".format(gy, gx))
    graph = [[0 for _ in range(gx)] for _ in range(gy)]
    for i, line in enumerate(mygraph[1:]):
        line = line.strip()
...
def valid(x, y):
    return (
        x >= 0 and x < gx and y >=0 and y < gy and 
        graph[x][y] != 'x' and 
